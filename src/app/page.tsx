...
export default function HomePage() {
    ...
    const onNavClick = (route: string) => {
        ...
        router.push(
            `/${route}?model=${model}&is_use_PAYPAL_AUTH_ASSERTION=${useAuthAssertion}`,
        );
    };

    const withPurchase = flows.filter((f) => f.category === "with_purchase");
    const withoutPurchase = flows.filter(
        (f) => f.category === "without_purchase",
    );
                ...
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            PayPal Advanced Checkout &amp; Vault — card, PayPal,
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-1.5 p-1 bg-slate-100 rounded-full">
                                {(["firstTime", "returning"] as const).map(
                                    (m) => (
                                        <button
                                            key={m}
                                            onClick={() =>
                                                setModel(m as VaultModel)
                                            }
                                            className={cn(
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
            ...
            <div className="space-y-4">
                {rows.map(({ label, party, color }) => {
                    ...
                    const hasPaypal =
                        party.paypal.vaultID || party.paypal.customerID;
